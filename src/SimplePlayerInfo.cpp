...
bool SimplePlayerInfo::setup(SimplePlayer* player) {
      ...
      if (m_slotIndex >= 0) {
            m_maturityLevel = Mod::get()->getSavedValue<int>("player_maturity_" + numToString(m_slotIndex), 0);
            m_orbsFeeded = Mod::get()->getSavedValue<int>("player_orbs_fed_" + numToString(m_slotIndex), 0);
      } else {
      ...
      if (m_slotIndex >= 0) {
            std::string key = "player_" + numToString(m_slotIndex);
      ...
      displayPlayer->setPosition({125, 100});
      ...
      if (m_nameInput) {
            ...
            if (m_slotIndex >= 0) {
                  savedName = Mod::get()->getSavedValue<std::string>(
                      "player_name_" + numToString(m_slotIndex), "");
            } else {
            ...
            if (m_slotIndex >= 0) {
                Mod::get()->setSavedValue("player_name_" + numToString(m_slotIndex), value);
            } else {
...
void SimplePlayerInfo::updateMaturityInfo() {
      ...
      maturityLabel->setPosition({125, 50});
      ...
      if (m_maturityLevel < 5) {
            if (m_currentRequirement == GrowthRequirement::ORB) {
                  reqText = "Needs: Orbs (" + numToString(m_orbsFeeded) + "/5)";
            } else if (m_currentRequirement == GrowthRequirement::STAR) {
                  int count = 0;
                  if (m_slotIndex >= 0)
                        count = Mod::get()->getSavedValue<int>("player_stars_fed_" + numToString(m_slotIndex), 0);
                  else
                        count = Mod::get()->getSavedValue<int>("player_stars_fed", 0);
                  reqText = "Needs: Stars (" + numToString(count) + "/5)";
            } else if (m_currentRequirement == GrowthRequirement::MOON) {
                  int count = 0;
                  if (m_slotIndex >= 0)
                        count = Mod::get()->getSavedValue<int>("player_moons_fed_" + numToString(m_slotIndex), 0);
                  else
                        count = Mod::get()->getSavedValue<int>("player_moons_fed", 0);
                  reqText = "Needs: Moons (" + numToString(count) + "/5)";
            } else if (m_currentRequirement == GrowthRequirement::DIAMOND) {
                  int count = 0;
                  if (m_slotIndex >= 0)
                        count = Mod::get()->getSavedValue<int>("player_diamonds_fed_" + numToString(m_slotIndex), 0);
                  else
                        count = Mod::get()->getSavedValue<int>("player_diamonds_fed", 0);
                  reqText = "Needs: Diamonds (" + numToString(count) + "/5)";
            } else {
                  reqText = "Fully Grown!\n(Needs: Diamonds)";
            }
      } else {
      ...
      requirementLabel->setPosition({125, 25});
